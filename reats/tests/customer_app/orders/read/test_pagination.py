...
class TestOrdersPaginationScenarios:
    ...
    def test_scenario_1_default_pagination(
    ...
    def test_scenario_4_second_page_with_enough_data(
        ...
        response_page1 = client.get(
            ...
            follow=False,
            **auth_headers,
            data={"status": OrderStatusEnum.PENDING, "page_size": 5}
        )
    ...
    def test_scenario_5_page_beyond_total_pages(
        ...
        response_info = client.get(
            ...
            follow=False,
            **auth_headers,
            data={"status": OrderStatusEnum.PENDING, "page_size": 10}
        )
    ...
    def test_scenario_7_no_orders_for_filter(
        self, auth_headers, client, customer_order_path, customer_with_30_orders
    ):
        customer, orders = customer_with_30_orders
    ...
    def test_scenario_8_combination_page_and_page_size(
        ...
        orders_sorted = list(OrderModel.objects
            .filter(customer=customer, status=OrderStatusEnum.PENDING)
            .order_by("-modified")
    ...
    def test_scenario_9_order_respected_across_pages(
        ...
        expected_order_ids = list(OrderModel.objects
            .filter(customer=customer, status=OrderStatusEnum.PENDING)
            .order_by("-modified")
            .values_list("id", flat=True))
    ...
    def test_scenario_10_total_items_consistent_across_pages(
        ...
        response_info = client.get(
            ...
            follow=False,
            **auth_headers,
            data={"status": OrderStatusEnum.PENDING, "page_size": 8}
        )
        response_info = client.get(
            customer_order_path,
            follow=False,
        ...
        direct_count = OrderModel.objects.filter(
            customer=customer,
            status=OrderStatusEnum.PENDING
        ).count()
        
        expected_total_items = next(iter(total_items_set))
        assert direct_count == expected_total_items, \
...
class TestPaginationEdgeCases:
    ...
    def test_pagination_with_existing_data_only(
        ...
        existing_orders_count = OrderModel.objects.filter(
            customer=customer,
            status=OrderStatusEnum.PENDING
        ).count()
    ...
    def test_pagination_response_structure_always_consistent(
        self, auth_headers, client, customer_order_path
    ):
        test_cases = [
            {"status": OrderStatusEnum.PENDING},
            {"status": OrderStatusEnum.PENDING, "page_size": 5},
